...
public interface DocumentRepository extends JpaRepository<Document, Long>, JpaSpecificationExecutor<Document> {
    ...
    @Query(value="select COUNT (*) FROM document d join user_document ud ON (d.id = ud.document_id) join type t ON (d.type_id=t.id) WHERE d.submitted_Date BETWEEN :startDate AND :endDate AND t.title =:title AND ud.submitted = true AND ud.rejected = false AND ud.confirmed = false AND d.type_id IN (SELECT t.id FROM type t JOIN type_group tg ON (t.id = tg.type_id) JOIN users_groups ug ON (tg.group_id=ug.group_id) JOIN user u ON (ug.user_id = u.id) JOIN ADMIN_GROUP g ON (tg.GROUP_ID =g.id) WHERE u.email=:email AND g.name=:name AND receive=true group by t.id)", nativeQuery = true)
    int findReceivedUserDocumentCountByGroupTypeDateRange(@Param("email") String email, @Param("startDate") Date startDate, @Param("endDate") Date endDate, @Param("title") String title, @Param("name") String name);
}
